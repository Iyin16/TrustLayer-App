...
import { Filter, Pencil, Loader2, Database, Plus, AlertCircle, RefreshCw, ArrowUpRight } from "lucide-react";
...
function TrustBadge({ score }: { score: number }) {
  ...
  return (
    <span className={["inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10.5px] font-semibold border tabular-nums", cls].join(" ")}>
  ...
  return (
    <Card>
      <div className="px-6 pt-6 pb-5 flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-[12.5px] text-[#a1a1aa]">
            {subtitle ??
              `${filtered.length}${status !== "All" ? ` of ${rows.length}` : ""} dataset${filtered.length === 1 ? "" : "s"} · sorted by trust score`}
          </p>
        </div>
        {showFilters && (
      ...
      <div className="border-t border-[#16161a]">
        <div className="grid grid-cols-[2fr_1.4fr_1.2fr_1fr_1fr_auto] px-6 py-3 text-[10.5px] font-semibold uppercase tracking-[0.14em] text-[#5a5a63] border-b border-[#16161a]">
          <div>Dataset</div>
          <div>Owner</div>
        ...
        {loading ? (
          ...
          filtered.map((ds) => (
            <div
              key={ds.name}
              className="group relative grid grid-cols-[2fr_1.4fr_1.2fr_1fr_1fr_auto] items-center px-6 py-4 border-b border-[#101014] last:border-b-0 hover:bg-[#101014]/60 transition-colors cursor-pointer"
            >
              {/* Clickable overlay — left-click navigates to detail */}
              <Link
              ...
              <div className="relative pointer-events-none">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[13.5px] font-medium text-white group-hover:text-[#ff7a59] transition-colors">
                    {ds.name}
                  </span>
                  <TrustBadge score={ds.trust} />
                </div>
              ...
              <div className="relative pointer-events-none flex items-center gap-2.5">
                ...
                <span className="text-[13px] text-[#d8d8de] truncate max-w-[120px]">{ds.ownerName}</span>
              </div>
              ...
              <div className="relative pointer-events-none flex items-center gap-2">
                ...
                <span
                  className={[
                    ...
                    ds.trust >= 80 ? "text-[#34d399]" : ds.trust >= 60 ? "text-[#fbbf24]" : "text-[#f87171]",
                  ].join(" ")}
                >
                  {ds.trust}
                </span>
              ...
              <div className="relative flex items-center justify-end gap-1">
                {onEdit && (
                  <button
                    ...
                    title="Edit dataset"
                    className="h-8 w-8 rounded-lg flex items-center justify-center text-[#5a5a63] hover:text-white hover:bg-[#16161a] transition-colors opacity-0 group-hover:opacity-100 pointer-events-auto z-10 relative"
                  >
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
                )}
                <ArrowUpRight className="h-4 w-4 text-[#3a3a40] group-hover:text-[#ff7a59] transition-colors" />
              </div>
            </div>
          ))
        )}
      ...
      {filtered.length > 0 && (
        <div className="px-6 py-2.5 border-t border-[#0d0d10] flex items-center gap-1.5 text-[11px] text-[#3a3a40]">
          ...
          Click a row to view full dataset details
        </div>
      )}
    </Card>
  );
